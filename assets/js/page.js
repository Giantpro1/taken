...
/* MARQUEE — cards already duplicated in HTML, just handle pause */
document.querySelectorAll('.marquee-row').forEach(row => {
  ...
  // Mobile: pause on touch, resume after 800ms so tap-to-link still works
  row.addEventListener('touchstart', () => row.classList.add('paused'), { passive: true });
  row.addEventListener('touchend', () => {
    setTimeout(() => row.classList.remove('paused'), 800);
  }, { passive: true });
});
